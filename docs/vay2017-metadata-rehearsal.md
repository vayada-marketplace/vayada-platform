...
1. Confirm the required infrastructure PR is merged and applied, and that the `next` GitHub environment is protected. The VAY-2043 runner IAM roles currently need a separately reviewed provisioning path; do not broaden the generic platform-deploy role to make apply pass.
...
The GitHub workflow role deliberately has no broad EC2, RDS, or ECR describe permissions. The operator check is therefore a required separate gate, not something the workflow silently skips. Repeat it immediately before every dispatch to minimize configuration drift.
