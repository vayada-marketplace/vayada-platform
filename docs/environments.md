...
| `/vayada/prod/openai-api-key`         | Temporarily retained during Ask runtime removal; no longer injected |
...
`TF_VAR_AUTH_COOKIE_SECRET`, and temporarily `TF_VAR_OPENAI_API_KEY` until the
retained OpenAI parameter is deleted after the runtime rollout.
