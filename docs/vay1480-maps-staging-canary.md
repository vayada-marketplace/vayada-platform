...
Each frontend activates after healthy rollout. Failures restore prior rule conditions/actions and task definitions; an initially created failed service/rule is removed. If the second frontend fails, the first successful frontend remains available. `next-maps-guest-remove` removes only the owned guest route and service so the test tenant falls back to the normal wildcard Booking service while preserving the admin preview. `next-maps-frontends-remove` removes both owned routes and services without changing baseline services. These actions share the isolated canary concurrency group. Service names retain the existing IAM-permitted `vayada-` prefix; permissions were not broadened.
