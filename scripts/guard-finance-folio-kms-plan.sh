...
phase="$(jq -er '
  ...
  [.resource_changes[]? | select(.type=="aws_kms_key" and .name=="finance_folio_recipient_fingerprint")] as $fingerprints |
  ...
  ($fingerprints[0].change.before.id//"") as $fingerprint_id |
  ...
  (($fingerprint_id|key_id) and $fingerprints[0].change.after.id==$fingerprint_id and $fingerprints[0].change.after.arn==("arn:aws:kms:eu-west-1:269416271598:key/"+$fingerprint_id) and (($fingerprints[0].change.after_unknown//{})|[..|select(.==true)]|length)==0) as $fingerprint_managed |
  if ($keys|length)!=1 or ($fingerprints|length)!=1 or ($aliases|length)!=1 or $keys[0].index!="v1" or $fingerprints[0].index!="v1" then error("ambiguous Finance KMS state")
  elif $keys[0].change.actions==["create"] and $fingerprints[0].change.actions==["create"] and $aliases[0].change.actions==["create"] and $keys[0].change.before==null and $fingerprints[0].change.before==null and $aliases[0].change.before==null then "finance-diagnostic"
  elif $managed and $fingerprint_managed and $keys[0].change.actions==["update"] and $fingerprints[0].change.actions==["update"] and $aliases[0].change.actions==["no-op"] then "finance-post-import"
  elif $managed and $fingerprint_managed and $keys[0].change.actions==["no-op"] and $fingerprints[0].change.actions==["no-op"] and $aliases[0].change.actions==["no-op"] then "finance-steady"
