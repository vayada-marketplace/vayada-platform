...
case "${mode}" in
  ...
  finance-steady)
    ...
    jq -e '
      ...
      $inventory.change.actions==["no-op"] and ($inventory.change.before|(has("ipc_mode") and has("pid_mode"))) and ($inventory.change.after|(has("ipc_mode") and has("pid_mode"))) and ($inventory.change.before|norm_inventory_modes)==($inventory.change.after|norm_inventory_modes) and (($inventory.change.after_unknown//{})|[..|select(.==true)]|length)==0 and ($inventory.change.after|stable_inventory)==$expected_inventory and
