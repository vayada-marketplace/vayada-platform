...
case "${mode}" in
  finance-diagnostic)
    ...
    if ! jq -e --arg version "${version}" '
      ...
      ($keys[0]) as $key | ($aliases[0]) as $alias | ($policies[0]) as $policy | ($tasks[0]) as $task | ($inventories[0]) as $inventory |
    ...
    ' <<<"${plan_json}" >/dev/null; then
      jq -c '[.resource_changes[] | select(.address=="aws_kms_key.finance_folio_recipient[\"v1\"]" or .address=="aws_kms_alias.finance_folio_recipient_current" or .address=="aws_iam_role_policy.next_api_finance_folio_recipient_kms" or .address=="aws_ecs_task_definition.services[\"next-target-backend\"]" or .address=="aws_ecs_task_definition.finance_folio_recipient_inventory") | {address,after_keys:(.change.after|keys),unknown_true_paths:([.change.after_unknown|paths(.==true)|map(tostring)|join(".")]|sort),known:{key_usage:.change.after.key_usage,custom_key_store_id:.change.after.custom_key_store_id,xks_key_id:.change.after.xks_key_id,timeouts:.change.after.timeouts,multi_region:.change.after.multi_region,description_type:(.change.after.description|type),tags:.change.after.tags}}]' <<<"${plan_json}" >&2
