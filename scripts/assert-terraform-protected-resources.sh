...
case "${mode}" in
  ...
  finance-steady)
    ...
    if ! jq -e '
      ...
      def norm_split_task_role: .task_role_arn=(if .task_role_arn==null then "" else .task_role_arn end);
      ...
      {container_definitions:$inventory.change.after.container_definitions,cpu:"256",enable_fault_injection:false,ephemeral_storage:[],execution_role_arn:"arn:aws:iam::269416271598:role/ecsTaskExecutionRole",family:"vayada-next-api-finance-folio-recipient-inventory",inference_accelerator:[],ipc_mode:null,memory:"512",network_mode:"awsvpc",pid_mode:null,placement_constraints:[],proxy_configuration:[],requires_compatibilities:["FARGATE"],runtime_platform:[],skip_destroy:false,tags:{Environment:"production",Project:"vayada",Purpose:"finance-folio-recipient-inventory"},tags_all:{Environment:"production",Project:"vayada",Purpose:"finance-folio-recipient-inventory"},task_role_arn:"",track_latest:false,volume:[]} as $expected_inventory |
      ...
      $alias.change.after=={arn:"arn:aws:kms:eu-west-1:269416271598:alias/vayada/prod/finance-folio-recipient-current",id:"alias/vayada/prod/finance-folio-recipient-current",name:"alias/vayada/prod/finance-folio-recipient-current",name_prefix:"",target_key_arn:$arn,target_key_id:$id} and
      ...
      (($inventory.change.actions==["no-op"] and ($inventory.change.before|norm_inventory_modes)==($inventory.change.after|norm_inventory_modes)) or
       ($inventory.change.actions==["delete","create"] and ($inventory.change.before|has("task_role_arn")) and $inventory.change.before.task_role_arn=="" and ($inventory.change.after|has("task_role_arn")) and $inventory.change.after.task_role_arn==null and (($inventory.change.before|stable_inventory|del(.container_definitions))==($inventory.change.after|stable_inventory|del(.container_definitions)|norm_split_task_role)) and
        ($inventory.change.before.container_definitions|fromjson)==[{command:["node","-e","console.log(JSON.stringify({status:\u0027INERT\u0027,message:\u0027use scripts/run-finance-folio-recipient-inventory.sh\u0027}))"],environment:[],essential:true,image:"269416271598.dkr.ecr.eu-west-1.amazonaws.com/vayada-next-api:next-latest",logConfiguration:{logDriver:"awslogs",options:{"awslogs-group":"/ecs/vayada-next-api","awslogs-region":"eu-west-1","awslogs-stream-prefix":"folio-recipient-inventory"}},mountPoints:[],name:"vayada-next-api-finance-folio-recipient-inventory",portMappings:[],secrets:[{name:"TARGET_DATABASE_URL",valueFrom:"arn:aws:ssm:eu-west-1:269416271598:parameter/vayada/prod/target-database-url"}],systemControls:[],volumesFrom:[]}])) and
      ($inventory.change.before|(has("ipc_mode") and has("pid_mode"))) and ($inventory.change.after|(has("ipc_mode") and has("pid_mode"))) and (($inventory.change.after_unknown//{})|del(.arn,.arn_without_revision,.id,.revision,.tags_all)|[..|select(.==true)]|length)==0 and
      (($inventory.change.actions==["no-op"] and ($inventory.change.after|stable_inventory)==$expected_inventory) or ($inventory.change.actions==["delete","create"] and ($inventory.change.after|stable_inventory|norm_split_task_role)==$expected_inventory)) and
