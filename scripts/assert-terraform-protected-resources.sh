...
case "${mode}" in
  ...
  finance-post-import)
    ...
    jq -e --arg version "${version}" '
      ...
      $fingerprint.change.after.arn==$fingerprint_arn and $fingerprint.change.after.id==$fingerprint_id and $fingerprint.change.after.key_id==$fingerprint_id and ($fingerprint.change.after.description|startswith("Finance folio recipient fingerprint "+$version+"; bootstrap=")) and $fingerprint.change.after.bypass_policy_lockout_safety_check==false and $fingerprint.change.after.custom_key_store_id==null and $fingerprint.change.after.customer_master_key_spec=="HMAC_256" and $fingerprint.change.after.deletion_window_in_days==30 and $fingerprint.change.after.enable_key_rotation==false and $fingerprint.change.after.rotation_period_in_days==null and $fingerprint.change.after.is_enabled==true and $fingerprint.change.after.key_usage=="GENERATE_VERIFY_MAC" and $fingerprint.change.after.multi_region==false and $fingerprint.change.after.xks_key_id==null and $fingerprint.change.after.timeouts==null and
      $fingerprint.change.after.tags=={Environment:"production",ManagedBy:"terraform",Name:("vayada-finance-folio-recipient-fingerprint-"+$version),Project:"vayada",Purpose:"finance-folio-recipient-fingerprint",Version:$version} and $fingerprint.change.after.tags_all==$fingerprint.change.after.tags and (($fingerprint.change.after.policy|fromjson|norm_policy)==($expected_fingerprint_policy|norm_policy)) and
  ...
  finance-steady)
    ...
    jq -e '
      ...
      ($fingerprint.change.after|keys)==["arn","bypass_policy_lockout_safety_check","custom_key_store_id","customer_master_key_spec","deletion_window_in_days","description","enable_key_rotation","id","is_enabled","key_id","key_usage","multi_region","policy","rotation_period_in_days","tags","tags_all","timeouts","xks_key_id"] and
      ($fingerprint_id|test("^[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}$")) and $fingerprint.change.after.arn==$fingerprint_arn and $fingerprint.change.after.key_id==$fingerprint_id and ($fingerprint.change.after.description|test("^Finance folio recipient fingerprint v1; bootstrap=.+$")) and $fingerprint.change.after.bypass_policy_lockout_safety_check==false and $fingerprint.change.after.custom_key_store_id==null and $fingerprint.change.after.customer_master_key_spec=="HMAC_256" and $fingerprint.change.after.deletion_window_in_days==30 and $fingerprint.change.after.enable_key_rotation==false and $fingerprint.change.after.rotation_period_in_days==null and $fingerprint.change.after.is_enabled==true and $fingerprint.change.after.key_usage=="GENERATE_VERIFY_MAC" and $fingerprint.change.after.multi_region==false and $fingerprint.change.after.xks_key_id==null and $fingerprint.change.after.timeouts==null and
