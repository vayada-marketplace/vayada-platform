...
def validate(sts, ssm, rds, ecs, recovery):
    ...
    parameter = ssm.get_parameter(Name=PARAMETER, WithDecryption=True)["Parameter"]
    if parameter["Name"] != PARAMETER or parameter["Type"] != "SecureString":
...
def rotate(ssm, rds, ecs, url, recovery):
    password = run_stage(
    ...
    run_stage("rotation_failed_service_update", lambda: ecs.update_service(
...
def main():
    ...
    url = validate(sts, ssm, rds, ecs, recovery)
    if args.mode == "check":
        print(json.dumps({"status": "PASS", "mode": "check", "recovery": recovery}))
        return
    rotate(ssm, rds, ecs, url, recovery)
