...
resource "aws_iam_role_policy" "vay2042_source_github" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      { Effect = "Allow", Action = "secretsmanager:DescribeSecret", Resource = [aws_secretsmanager_secret.vay2042_source_reader.arn, aws_secretsmanager_secret.vay2042_target_writer.arn] },
    ]
  })
}
